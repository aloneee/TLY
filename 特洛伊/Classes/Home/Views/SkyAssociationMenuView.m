...
- (instancetype)init
{
    ...
    if (self) {
        ...
        cancelBtn.frame = CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 64);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == [tables objectAtIndex:0]){
        ...
        cell.imageView.image = [_delegate assciationMenuView:self imageForClass_1:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == t0){
        ...
        if(isNexClass) {
            [self saveSels];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CHOSECARFINISHED" object:nil];
            [self dismiss];
